...
export function createMcpServer(ctx: McpContext): McpServer {
  ...
  server.registerTool(
    "test_status",
    {
      title: "Test status",
      description:
        `Summary of the most recent test run reported by the Claude Code harness. This does NOT run ` +
  ...
  server.registerTool(
    "execution_summary",
    {
      ...
      description:
        `Recent Claude Code execution records for this workspace: task id, iteration, changed files, ` +
        `tests and exit status. Use it after Claude Code reports EXECUTED. ${UNTRUSTED_NOTE}`,
      inputSchema: {
  ...
  server.registerTool(
    "execution_output",
    {
      ...
      description:
        `List or read command output that Claude Code chose to record after a test/build/lint/typecheck ` +
